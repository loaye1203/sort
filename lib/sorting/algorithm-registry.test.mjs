...
const algorithmsSource = ["full", "simplified", "simulated"]
  .map((level) => readFileSync(new URL(`./algorithms/${level}.ts`, import.meta.url), "utf8"))
  .join("\n");
...
test("implemented batch ids are registered and implemented", () => {
  for (const id of implementedIds) {
    ...
    const registryBlock = registrySource.match(new RegExp(`id: "${id}"([\\s\\S]*?)(?:\\n  },\\n  \\{|\\n  },\\n];)`));
    assert.ok(registryBlock, `${id} registry block is missing`);
    assert.match(registryBlock[1], /load: (?:loadFullAlgorithm|loadSimplifiedAlgorithm|loadSimulatedAlgorithm|null)/);
    if (registryBlock[1].includes("load: null")) {
      assert.match(registryBlock[1], /catalogCode: `/, `${id} is missing catalog pseudocode`);
    } else {
...
test("each algorithm has one simple user-facing description", () => {
  const registeredIds = [...registrySource.matchAll(/id: "([^"]+)"/g)].map((match) => match[1]);
  const descriptionEntries = [...registrySource.matchAll(/description: "([^"]+)"/g)];
  const descriptions = new Map(registeredIds.map((id, index) => [id, descriptionEntries[index]?.[1]]));
